...
let display bsp p =
  ...
  in let doParse s =
          ...
          (truncate (float_of_int (!rs.porig.y) +. float_of_int (1 - !rs.porig.x) *. (angle !rs)))
          !rs.pdest.x !rs.pdest.y
          else if !rs.pdest.x < 1 then rs := new_segment !rs.porig.x !rs.porig.y 1 (truncate (float_of_int (!rs.pdest.y) +. (float_of_int (1 - !rs.pdest.x)) *. (angle !rs)))
